...
namespace ararararargibot
{
    public static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                string ex_dir = Directory.GetCurrentDirectory();

                string ex_file = Directory.GetCurrentDirectory() + "/Exceptions.txt";

                if (!Directory.Exists(ex_dir))
                {
                    Directory.CreateDirectory(ex_dir);

                    if (!File.Exists(ex_file))
                    {
                        System.IO.File.Create(ex_file);
                    }
                }

                using (StreamWriter sw = File.AppendText(ex_file))
                {
